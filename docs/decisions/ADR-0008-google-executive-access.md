...
`EXECUTIVE_MEMBER_MAP`, whose target must be an executive or the named executive
assistant in the synthetic directory. The server derives the acting persona from that mapping on every
...
executive or assistant cannot sign in until an administrator maps their verified email.
Private workspace and assessment state remain scoped to the authenticated
member. Approval and meeting records are shared across mapped members so
named reviewers can complete a handoff; each read and decision still checks
the acting persona's access. Sign-in and role authorization still need a live
Google/Vercel test after credentials are configured; local tests cannot
simulate Google's consent flow.
