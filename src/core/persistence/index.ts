...
export async function runDatabaseSession<T>(id: string, operation: () => Promise<T>, db: DatabasePool = database()): Promise<T> {
...
export function withDemoState<Args extends unknown[]>(handler: (request: Request, ...args: Args) => Promise<Response>) {
  return async (request: Request, ...args: Args): Promise<Response> => {
    ...
    try {
      const result = await (useMemory() ? runMemorySession(id, () => handler(request, ...args)) : runDatabaseSession(id, () => handler(request, ...args)));
